...
router.get('/getPosts',(req,res)=>{
    axios.get('https://'+req.hostname+'/api/posts/')
        .then(dados => res.jsonp(dados.data))
        .catch(erro => {res.status(500).send("Erro a obter os posts")})
})

router.post('/like/:id',(req,res)=>{
    axios.post('http://'+req.hostname+'/api/posts/like/'+req.params.id, {}, {headers: {cookie: req.headers.cookie}})
...
router.post('/dislike/:id',(req,res)=>{
    axios.post('http://'+req.hostname+'/api/posts/dislike/'+req.params.id, {}, {headers: {cookie: req.headers.cookie}})
...
router.post('/opinion/:id',(req,res)=>{
    ...
    axios.post('http://'+req.hostname+'/api/posts/opinions/'+req.params.id, req.body, {headers: {cookie: req.headers.cookie}})
...
router.post('/novoPost/:id', (req,res)=>{
    ...
    form.parse(req,(erros,fields,files)=>{
        ...
        fs.rename(fenviado,fnovo,err=>{
            if(!err){
                axios.post("http://"+req.hostname+"/api/bikes/", bike, {headers: {cookie: req.headers.cookie}})
                    .then(resposta => {
                        ...
                        post.picture = fnovo
                        axios.post("http://"+req.hostname+"/api/posts/", post, {headers: {cookie: req.headers.cookie}})
                            .then(resposta=>{
                                res.redirect('/homeOn')
                            })    
                            .catch(erro => {
